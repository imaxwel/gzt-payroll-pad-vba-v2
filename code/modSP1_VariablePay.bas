...
Private Sub ProcessExtraTable(ws As Worksheet, empIndex As Object)
    ...
    If headerRow = 0 Then
        LogWarning "modSP1_VariablePay", "ProcessExtraTable", "Header row with WEIN not found in [特殊奖金] sheet"
        Exit Sub
    End If
    ...
    LogInfo "modSP1_VariablePay", "ProcessExtraTable", "Processed Extra Table [特殊奖金] sheet (header at row " & headerRow & ")"
    Exit Sub
